...
const Organization = () => {
  ...
  const getOrganizationById = async (id) => {
    try {
      const response = await axios.get(`${config.BASE_URL}/organization/${id}`);
  ...
  const deleteData = async () => {
    try {
      await axios.delete(`${config.BASE_URL}/organization/${idOrganization}`, {
  ...
  const addOrganization = () => {
    navigate(`/admin/struktur-pemerintahan/add`);
  };

  const editOrganization = (id) => {
    navigate(`/admin/struktur-pemerintahan/edit/${id}`);
  };
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        Header: 'Foto',
        accessor: 'imageUrl',
        Cell: ({ value }) => <img src={value} alt="Foto Organization" />,
      },
      {
        Header: 'Aksi',
        Cell: ({ row }) => (
          <div className="aksi">
            <button onClick={() => editOrganization(row.original._id)}>
